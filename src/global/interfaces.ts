...
export class SuccessResponse<T = {}> extends ServiceResponse<T> {
  ...
  constructor(public payload = {} as T, public status = 200, public message = 'successful') {
    super(payload, status, message);
  }
}

export class ServiceUnavailableResponse<T> extends ServiceResponse<T> {
  public type: ResponseTypes.unavailable = ResponseTypes.unavailable;
  constructor(
    public payload = {} as T,
    public status = 503,
    public message = 'Service Currently Unavailable'
  ) {
    super(payload, status, message);
  }
}
...
export class NotImplementedResponse<T> extends ServiceResponse<T> {
  ...
  constructor(public payload = {} as T, public status = 501, public message = 'not_implemented') {
    super(payload, status, message);
  }
}
...
export class UnauthorizedResponse<T> extends ServiceResponse<T> {
  public type: ResponseTypes.notImplemented = ResponseTypes.notImplemented;
  constructor(public payload = {} as T, public status = 401, public message = 'unauthorized') {
    super(payload, status, message);
  }
}
